WITH LatestAppFilter AS (
    ...
    INNER JOIN "Household" AS H 
        ON A."ID" = H."ApplicationID"
    LEFT JOIN "Benefit" AS B 
        ON A."ID" = B."ApplicationID"
    LEFT JOIN "BenefitFamily" AS BF 
        ON B."ID" = BF."BenefitID" 
       AND H."SSN" = BF."SSN"
    WHERE A."ApplicationTypeID" = 1
      ...
      AND (
            ...
            OR EXISTS (
                SELECT 1
                FROM "Household" H2
                LEFT JOIN "Benefit" B2 
                    ON H2."ApplicationID" = B2."ApplicationID"
                LEFT JOIN "BenefitFamily" BF2 
                    ON B2."ID" = BF2."BenefitID" 
                   AND H2."SSN" = BF2."SSN"
...
SELECT
    ...
    CASE
        WHEN COALESCE("BF"."IsPrimaryApplicant", "H"."IsPrimaryApplicant") = 't' THEN 'այո'
        WHEN COALESCE("BF"."IsPrimaryApplicant", "H"."IsPrimaryApplicant") = 'f' THEN 'ոչ'
        ELSE NULL
    END AS "3",
    ...
    CASE
        WHEN COALESCE("BF"."HasDisability", "H"."HasDisability") = 't'
          OR COALESCE("BF"."HasFunctionalLimit", "H"."HasFunctionalLimit") = 't'
        THEN 1
        ELSE 0
    END AS "6",
    ...
    CASE
        WHEN COALESCE("BF"."HasDependentChild", "H"."HasDependentChild") = 't' THEN 1
        ELSE 0
    END AS "10",
    ...
    CASE
        WHEN COALESCE("BF"."IsRegisteredInEWork", "H"."IsRegisteredInEWork") = 't' THEN 1
        ELSE 0
    END AS "17",
    ...
    COALESCE("BF"."RealEstateNetIncomeAmount", "H"."RealEstateNetIncomeAmount") AS "31",
    ...
    (
        ...
        COALESCE("BF"."UrgentSupportAmount", "H"."UrgentSupportAmount", 0) +
        COALESCE("BF"."ArtsakhSupportAmount", "H"."ArtsakhSupportAmount", 0) +
        COALESCE("B"."BenefitAmount", 0) / NULLIF("EMC"."EligibleCount", 0)
    ) AS "35",

    CASE
        WHEN COALESCE("BF"."LoanRepaymentAmount", "H"."LoanRepaymentAmount") IS NOT NULL THEN 1
        ELSE 0
    END AS "44",

    COALESCE("BF"."LoanRepaymentAmount", "H"."LoanRepaymentAmount") AS "45"

FROM "Application" AS "App"
INNER JOIN LatestAppFilter AS "LAF"
    ON "App"."ID" = "LAF"."TargetAppID"
INNER JOIN "Household" AS "H"
    ON "App"."ID" = "H"."ApplicationID"
LEFT JOIN "Benefit" AS "B"
    ON "App"."ID" = "B"."ApplicationID"
LEFT JOIN "BenefitFamily" AS "BF"
    ON "B"."ID" = "BF"."BenefitID"
   AND "H"."SSN" = "BF"."SSN"
LEFT JOIN EligibleMemberCount AS "EMC"
    ON "App"."ID" = "EMC"."ApplicationID"
WHERE "LAF"."rn" = 1
