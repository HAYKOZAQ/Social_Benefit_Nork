WITH LatestAppFilter AS (
    SELECT 
        ...
        -- 1. Get the very first submission date for this family/SSN
    ...
    WHERE A."ApplicationTypeID" = 1
        ...
        -- Updated Logic:
        AND (
            -- Case A: The Primary Applicant is present
            COALESCE(BF."AbsenceReasonID", H."AbsenceReasonID") IS NULL
            OR 
            -- Case B: Primary is absent, but someone else in the household is present
            EXISTS (
                SELECT 1 
                FROM "Household" H2
...
-- 	COUNT(DISTINCT CASE WHEN "App"."Status" IN (5, 21) THEN "BF_MAIN"."SSN" END) AS "40", 
...
END) AS "Count_40",
    ...
    COALESCE(bf."RentalAssistanceAmount", h."RentalAssistanceAmount", 0) +
          ...
          COALESCE(bf."ArtsakhSupportAmount", h."ArtsakhSupportAmount", 0)) AS "Total_66",
      sum( 
    COALESCE(bf."RentalAssistanceAmount", h."RentalAssistanceAmount", 0) +
          COALESCE(bf."MigrantSupportAmount", h."MigrantSupportAmount", 0) +
          COALESCE(bf."RefugeeSupportAmount", h."RefugeeSupportAmount", 0) + 
	...
	LEFT JOIN (
		SELECT
			"AL"."ApplicationID", 
			SUM(CASE WHEN "LT"."Name" = 'Կով' THEN "AL"."Count" ELSE 0 END) AS "53", SUM(CASE WHEN "LT"."Name" = 'Այլ սեռահասակային խմբերի տավար' THEN "AL"."Count" ELSE 0 END) AS "54", 
			SUM(CASE WHEN "LT"."Name" = 'Գոմշամատակ' THEN "AL"."Count" ELSE 0 END) AS "55", SUM(CASE WHEN "LT"."Name" = 'Խոզամայր (խոճկորներով)' THEN "AL"."Count" ELSE 0 END) AS "56", 
			SUM(CASE WHEN "LT"."Name" = 'Այլ սեռահասակային խմբերի խոզեր' THEN "AL"."Count" ELSE 0 END) AS "57", SUM(CASE WHEN "LT"."Name" in ('Ոչխար', 'Այծ') THEN "AL"."Count" ELSE 0 END) AS "58", 
			 SUM(CASE WHEN "LT"."Name" = 'Հավ' THEN "AL"."Count" ELSE 0 END) AS "60", 
			SUM(CASE WHEN "LT"."Name" in ('Բադ','Սագ') THEN "AL"."Count" ELSE 0 END) AS "61", 
			SUM(CASE WHEN "LT"."Name" = 'Ջայլամ' THEN "AL"."Count" ELSE 0 END) AS "63", SUM(CASE WHEN "LT"."Name" = 'Հնդկահավ' THEN "AL"."Count" ELSE 0 END) AS "64", 
			SUM(CASE WHEN "LT"."Name" = 'Մեղվաընտանիք' THEN "AL"."Count" ELSE 0 END) AS "65", SUM(CASE WHEN "LT"."Name" = 'Ճագար' THEN "AL"."Count" ELSE 0 END) AS "66"
		FROM "ApplicationLivestock" AS "AL" JOIN "LivestockType" AS "LT" ON "AL"."LivestockTypeID" = "LT"."ID"
...
GROUP BY
	"App"."Num", "App"."Content" -> 'CitizenData' -> 'ActualAddress' ->> 'Region', 
	"App"."Content" -> 'CitizenData' -> 'ActualAddress' ->> 'Community', "App"."Status", "Benefit"."BenefitStatusID", "App"."ID", "App"."LivestockIncome", "Benefit"."OrderDate", "LAF"."Total_Applications",
